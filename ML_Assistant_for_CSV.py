...
option = st.selectbox('How would you like to be contacted?',('Clean your dataset', 'Extract features from dataset', 'Augment the biased dataset', 'Build a model just by uploading unprocessed dataset'))

if option == 'Clean your dataset':
    st.title("Data cleaning")
    st.markdown("This will help you to clean your dataset and remove all null values or NaN values and does all basic cleaning that is to be done for a dataset. And it also label encoding for the output column as well")
    ...
    if input_c:
        ...
        st.download_button("Press to Download", csv_cleaned,
                        "cleaned_dataset.csv", "text/csv", key='download-csv')


if option == 'Extract features from dataset':
    st.title("Feature extraction")
    st.markdown("This helps you to extract required features which will will eventually help you in model building which is done by checking correlation between the columns and even with the output column, and this also converts all non-numeric/categorical/continuous type into numeric datas. The output dataset can directly be used for model building.")
    ...
    if input_fe:
        ...
        st.download_button("Press to Download", csv_fe,
                        "dataset_with_useful_features.csv", "text/csv", key='download-csv')


if option == 'Augment the biased dataset':
    st.title("Dataset Augmentation")
    st.markdown("This helps you to augment a given biased dataset and this will return you a un-biased dataset.")
    ...
    if input_a:
        with st.spinner("Augmenting Dataset..."):
            ...
            csv_fe = df_fe.to_csv().encode('utf-8')
        st.download_button("Press to Download", csv_fe,
                        "unbiased_dataset.csv", "text/csv", key='download-csv')
        st.markdown(exp)

if option == 'Build a model just by uploading unprocessed dataset':
    st.title("Model Building")
    st.markdown("This helps you to build a model for the uploaded dataset. And all the above operations are also done in this, you don't have do pre-processing for the dataset. Just upload a raw CSV and get the model as a .h5 file with the accuracies printed below.")
    ...
    if input_m:
        with st.spinner("Building model..."):
            ...
            st.download_button("Press to Download", pkl_file, "model.pkl",
